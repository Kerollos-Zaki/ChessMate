...
class _GameplayScreenState extends State<GameplayScreen> {
  ...
  void _onSquareTap(int index) {
    setState(() {
      ...
      if (selectedIndex == null) {
        ...
        if (success) {
          SoundService.playButtonSound(context);
          _dbRef.child('pending_move').set("$fromSquare$toSquare");
    ...
    return Scaffold(
      ...
      body: StreamBuilder<DocumentSnapshot>(
        ...
        builder: (context, userSnapshot) {
          ...
          return Stack(
            children: [
              Positioned(top: -100, right: -100, child: Container(width: 300, height: 300, decoration: BoxDecoration(shape: BoxShape.circle, color: Colors.white.withOpacity(0.03)))),
              Column(
                children: [
                  SafeArea(
                    child: Padding(
                      ...
                      child: Row(
                        children: [
                          ...
                          const Text('Match', style: TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.w600)),
                        ],
                      ),
                    ),
                  ),
                    ...
                    child: Column(
                      children: [
                        ...
                        AspectRatio(
                          aspectRatio: 1,
                          child: Container(
                            decoration: BoxDecoration(borderRadius: BorderRadius.circular(16), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.5), blurRadius: 20, spreadRadius: 5)]),
                            child: ClipRRect(
                              ...
                              child: GridView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  return GestureDetector(
                                    ...
                                    child: Container(
                                      color: isLight ? Colors.white.withOpacity(0.15) : Colors.white.withOpacity(0.05),
                                      child: Stack(
                                        alignment: Alignment.center,
                                        children: [
                                          if (isSelected) Container(color: Colors.yellow.withOpacity(0.3)),
                                          if (isValidMove) Container(width: 12, height: 12, decoration: const BoxDecoration(shape: BoxShape.circle, color: Colors.white38)),
                        ...
                        const SizedBox(height: 20),
                        _buildPlayerInfo(username, isWhite: true, photoUrl: photoUrl),
                      ],
                    ),
                  ),
                ],
  ...
  Widget _buildPlayerInfo(String name, {required bool isWhite, String? photoUrl}) {
    return Container(
          ...
          Row(
            children: [
              Container(
                ...
                child: photoUrl != null
                    ? ClipOval(child: photoUrl.startsWith('http') ? Image.network(photoUrl, fit: BoxFit.cover) : Image.asset(photoUrl, fit: BoxFit.cover))
                    : Icon(isWhite ? Icons.person : Icons.computer, color: Colors.white, size: 20),
              ),
